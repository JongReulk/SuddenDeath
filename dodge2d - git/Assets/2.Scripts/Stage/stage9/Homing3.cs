...
public class Homing3 : MonoBehaviour
{
    private Transform Target1;
    private Transform Target2;
    private float MoveSpeed = 420f;
    private float RotateSpeed = 2000f;

    private int playernum;
    private float StartTime = 8f;
    public GameObject player1_ho_3;
    public GameObject player2_ho_3;
    private float[] homings = new float[2];
    private Rigidbody2D Homingrb;
    private float HomingTime3;
    ...
    void FixedUpdate()
    {
        ...
        HomingTime3 += Time.deltaTime;
        //print("Homing Start :" + StartTime);
        if (HomingTime3 > StartTime)
        {
            if (playernum == 1)
            {
        ...
        if (!player1_ho_3.activeInHierarchy)
        {
            Target1 = Target2;
        }

        if (!player2_ho_3.activeInHierarchy)
        {
            Target2 = Target1;
        }
        if (!player2_ho_3.activeInHierarchy && !player1_ho_3.activeInHierarchy)
        {
